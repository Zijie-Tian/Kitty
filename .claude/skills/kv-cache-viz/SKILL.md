...
description: >-
  ...
  "画图 / 可视化 / probe / 分析 KV cache / 看分布 / LongBench 结果图"
  等，就应该使用本 skill，而不是手动拼 `python scripts/dump_*.py` 或
  `python scripts/plot_*.py` 命令。本 skill 把 22 个随手脚本统一成
...
| `probe signpt-dequant` | `dump_signpt_dequant_dist.py` | qlutattn (sign channels) 反量化前后 channel 分布对比 | `.png` |
| `probe sign-scale` | `dump_sign_scale_dist.py` | qlutattn sign-group scale 分布 + 16-token 共享实验 | `.pt`, `.json`, `.png` |
| `probe v-grouping-error` | `dump_v_grouping_error.py` | V 量化分组（per-token vs token/channel block）误差研究 | `.json` |
...
| `viz decomp` | `plot_k_decomp_steps.py` | K = μ + residual + qlutattn sign 重建的 3D 图 |
...
bash .claude/skills/kv-cache-viz/scripts/run_kv_cache_viz.sh probe mu2sigma2 \
  ...
  --longbench-dir "$HOME/data/LongBench/data" \
  --seq-len 1024 --tag llama32-1b --outdir probe_out/sign_scale

# 2. dump layer 8 的 K/V 用于离线画图
bash .claude/skills/kv-cache-viz/scripts/run_kv_cache_viz.sh dump-layer \
  --layer 8 --model "$HOME/models/Llama-3.2-1B-Instruct" \
  --longbench-dir "$HOME/data/LongBench/data" \
